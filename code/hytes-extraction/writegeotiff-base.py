...
if __name__ == '__main__':
    raw_hy_dir_path = r'' # the directory that contains the hdf5 file, the geo.dat and the geo.hdr.
    hdf_f_path = os.path.join(raw_hy_dir_path,'<name_of_scene>.hdf5')
    geo_dat_f_path = os.path.join(raw_hy_dir_path,'<name_of_scene>.geo.dat')
    geo_hdr_f_path = os.path.join(raw_hy_dir_path,'<name_of_scene>.geo.hdr')

    data,lat,lon = hytesinputs(hdf_f_path,geo_dat_f_path,geo_hdr_f_path)
    dst_dir = r''
    dst_path = os.path.join(dst_dir,'<name_of_desired_written_scene>.tif')
